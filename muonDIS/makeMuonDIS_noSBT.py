...
def inspect_file(filename):
    ...
    for i, event in enumerate(tree):
        ...
        nSSThits = event.muon_strawtubePointss.GetEntries()
        nUBThits = event.muon_UpstreamTaggerPoints.GetEntries()

        table_rows.append(
            [i, fix_target, nParticles, nSoftTracks, nSSThits, nUBThits, cross_sec]
        )
...
def makeMuonDIS():
    ...
    muon_strawtubePointss = r.TClonesArray("strawtubePoints")
    output_tree.Branch("muon_strawtubePointss", muon_strawtubePointss, 32000, -1)
    ...
    for k in range(first_mu_event, last_mu_event):
        ...
        for a in range(args.nDIS):
            ...
            muon_strawtubePointss.Clear()

            index = 0
            for hit in muon_tree.muon_strawtubePointss:
                if muon_strawtubePointss.GetSize() == index:
                    muon_strawtubePointss.Expand(index + 1)
                hit.SetTrackID(0)  # Set TrackID to match for muon ID for new simulation
                muon_strawtubePointss[index] = hit
                index += 1
