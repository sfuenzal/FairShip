#!/usr/bin/env python
"""Script to add the incoming muon's MC hits in the SST to the simulation file."""
...
headers = [
    "Event",
    "Muon strawtubePoints Available",
    "Original strawtubePoints Count",
    "Muon strawtubePoints Added",
    "Combined strawtubePoints Count",
...
def inspect_file(inputfile, muonfile, print_table=False):
    """Inspecting the produced file for successfully added muon SST points."""
    ...
    for i, (muon_event, input_event) in enumerate(zip(muon_tree, input_tree)):
        muon_hits = len(muon_event.muon_strawtubePoints)
        muoncount = 0
        if muon_hits:
            for hit in input_event.strawtubePoint:
                if hit.GetTrackID() == 0:
                    muons_found = True
                    muoncount += 1
        ...
        table_data.append(
            [
                i,
                muon_hits,
                len(input_event.strawtubePoint) - muoncount,
                muoncount,
                len(input_event.strawtubePoint),
                muon_ubthits,
...
def modify_file(inputfile, muonfile):
    ...
    logging.warning(
        f"strawtubePoints & UpstreamTaggerPoints from the incoming muon (saved in {muonfile}) will be added to {inputfile}."
    )
    ...
    # Open the external file with additional strawtubePoints
    muon_file = r.TFile.Open(muonfile, "read")
    try:
        muon_tree = muon_file.DIS
    except Exception as e:
    ...
    combined_strawtubePoint = r.TClonesArray("strawtubePoint")
    output_tree.SetBranchAddress("strawtubePoint", combined_strawtubePoint)
    ...
    for i, (input_event, muon_event) in enumerate(zip(input_tree, muon_tree)):
        ...
        combined_strawtubePoint.Clear()

        index = 0

        for hit in input_event.strawtubePoint:
            if combined_strawtubePoint.GetSize() == index:
                combined_strawtubePoint.Expand(index + 1)
            combined_strawtubePoint[index] = hit  # pending fix to support ROOT 6.32+
            index += 1

        muoncount = 0
        for hit in muon_event.muon_strawtubePoints:
        ...
        table_data.append(
            [
                i,
                len(muon_event.muon_strawtubePoints),
                len(input_event.strawtubePoint),
                muoncount,
                len(combined_strawtubePoint),
...
if not muons_found:
    print(
        "Incoming muon's strawtubePoints & UpstreamTaggerPoints inf missing in file, proceeding with modification"
    )
