#!/usr/bin/env python
"""Script to collect muons hitting the SST (including soft interaction products) to a ROOT file."""
...
parser.add_argument(
    "-o",
    "--outputfile",
    default="muonsProduction_wsoft_SST.root",
...
if args.testing_code:
    logging.info(
        "test code, output file name overwritten as: muonsProduction_wsoft_SST_test.root"
    )
    args.outputfile = "muonsProduction_wsoft_SST_test.root"
    selectedmuons = "SelectedMuonsSST_test.txt"
else:
    selectedmuons = "SelectedMuonsSST.txt"
...
muon_strawtubePoints = r.TClonesArray("strawtubePoint")
output_tree.Branch("muon_strawtubePoints", muon_strawtubePoints)
...
h["PvPt_muon"] = r.TH2F(
    "PvPt_muon",
    "The momentum of the muons hitting the SST(unweighted);P(GeV/c);Pt(GeV/c)",
    200,
    0.0,
    400,
    40,
...
h["n_muon"] = r.TH2I(
    "n_muon",
    "Number of muons hitting the SST per Event;number of mu- per event (unweighted) bin width =1;number of mu+ per event (unweighted) bin width=1",
    6,
    0.0,
    6,
    6,
...
h["n_sbthits"] = r.TH1I(
    "n_sbthits", "Number of SST hits per muon;n_sbthits(unweighted);", 900, 0, 900
)
...
for inputFolder in os.listdir(path):
    ...
    for eventNr, event in enumerate(tree):
        ...
        muon_ids = []
        muon_hits = {}
        # Collect track IDs of muons which hit the SST
        for hit in event.strawtubePoint:
        ...
        for muon_ in muon_ids:
            ...
            for hit in event.strawtubePoint:
                ...
                if (
                    ...
                    if muon_strawtubePoints.GetSize() == index:
                        muon_strawtubePoints.Expand(index + 1)
                    muon_strawtubePoints[index] = hit
                    ...
                    ):  # only save the info of first SST  hit
                    ...
                    muon_table[-1][-2] = len(muon_strawtubePoints)

            h["n_sbthits"].Fill(len(muon_strawtubePoints))
...
with r.TFile.Open(args.outputfile, "read") as file:
    ...
    for event in tree:
        ...
        num_muonhits = len(event.muon_strawtubePoints)
