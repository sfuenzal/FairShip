...
parser.add_argument(
    "--hnl-production-yaml",
    dest="hnl_production_yaml",
    default=None,
    help=(
        "Optional HNL production YAML. When set, exports HNL_PRODUCTION_YAML "
        "before importing pythia8_conf, so custom B/Bc -> mu HNL definitions "
        "can be used without overwriting $FAIRSHIP/python/hnl_production_test.yaml."
    ),
)
parser.add_argument(
    "--NuRadio",
...
if options.hnl_production_yaml:
    hnl_yaml = os.path.abspath(os.path.expandvars(options.hnl_production_yaml))
    if not os.path.exists(hnl_yaml):
        parser.error(f"--hnl-production-yaml does not exist: {hnl_yaml}")
    os.environ["HNL_PRODUCTION_YAML"] = hnl_yaml
    print(f"HNL production YAML override: {hnl_yaml}")
...
if options.A != "c":
    inclusive = options.A
    if options.A in ("b", "bc"):
        # Bc is a b-flavoured hadron and is contained in the MSEL=5 beauty
        # cascade.  Never fall back to the default charm cascade for -A bc.
    ...
    if HNL or options.RPVSUSY:
        ...
        if HNL:
            ...
            if inclusive in ("b", "bc"):
                parent_label = "B+/-" if inclusive == "b" else "Bc+/-"
                print(
                    f"Charged-parent HNL mode: {parent_label} -> mu+/- HNL "
                    "(both charges supplied by PYTHIA charge conjugation)"
                )
