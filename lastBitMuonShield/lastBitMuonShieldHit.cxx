#include "lastBitMuonShieldHit.h"
...
// -----   Default constructor   --------------
lastBitMuonShieldHit::lastBitMuonShieldHit() : ShipHit() {
  flag = true;
}

// -----   constructor from lastBitMuonShieldPoint from lastBitMuonShieldHit-------------------------------
lastBitMuonShieldHit::lastBitMuonShieldHit(lastBitMuonShieldPoint* p, Double_t t0) : ShipHit() {
...
// -----   Destructor   -------------------------
lastBitMuonShieldHit::~lastBitMuonShieldHit() {}

// ---- return time information for a given track extrapolation
std::vector<double>  lastBitMuonShieldHit::GetTime(Double_t x) {
...
std::vector<double>  lastBitMuonShieldHit::GetTime() {
  TGeoBBox* shape =  (TGeoBBox*)gGeoManager->GetVolume("lastBitMuonShield")->GetShape();
...
// -----   resolution function-------------------
Double_t lastBitMuonShieldHit::Resol(Double_t x) {
  return par[0]*TMath::Exp( (x-par[2])/par[1] )+par[3];
}

std::vector<double> lastBitMuonShieldHit::GetMeasurements(){
  std::vector<double> m;
  m.push_back( t_1);
  m.push_back( t_2);
  return m;
...
// distance to edges
void lastBitMuonShieldHit::Dist(Float_t x, Float_t& lpos, Float_t& lneg) {
...
// ----------------------------------------------
TVector3 lastBitMuonShieldHit::GetXYZ() {
...
Double_t lastBitMuonShieldHit::GetX() {
  TVector3 pos = GetXYZ();
  return pos.X();
}

Double_t lastBitMuonShieldHit::GetY() {
  TVector3 pos = GetXYZ();
  return pos.Y();
}

Double_t lastBitMuonShieldHit::GetZ() {
  TVector3 pos = GetXYZ();
  return pos.Z();
}

TGeoNode* lastBitMuonShieldHit::GetNode() {
   TGeoNavigator* nav = gGeoManager->GetCurrentNavigator();
   //TString path = "/LastBitMuonShield_Detector_1/lastBitMuonShield_";path+=fDetectorID;
   //Bool_t rc = nav->cd(path);
   return nav->GetCurrentNode();
}

// -----   Public method Print   -----------------------
void lastBitMuonShieldHit::Print() const {
  cout << "-I- lastBitMuonShieldHit: lastBitMuonShield hit " << " in detector " << fDetectorID << endl;
